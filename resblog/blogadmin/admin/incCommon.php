<?php
	...
	// kiểm tra xem thiết lập ban đầu đã được thực hiện hay chưa
	detect_config();
	migrate_config();
	...
	// kiểm tra xem hệ thống thành viên có tồn tại không
	setupMembership();


	########################################################################

	// yêu cầu đăng xuất quản trị viên không?
	if(isset($_GET['signOut'])){
		logOutUser();
		redirect('../index.php?signIn=1');
	}

	// có người dùng đã đăng nhập không?
	if(!$uname=getLoggedAdmin()){
		// có người dùng đang cố gắng đăng nhập không?
