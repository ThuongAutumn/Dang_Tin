<?php
	// kiểm tra MD5 của tệp này để đảm bảo rằng nó đã không được gọi trước đó
	...
	function setupTable($tableName, $createSQL='', $silent=true, $arrAlter=''){
		...
		// có một truy vấn đổi tên bảng không?
		...
		if($res=@db_query("select count(1) from `$tableName`")){ // bảng đã tồn tại
		...
		}else{ //tableName đã cho không tồn tại

			if($oldTableName!=''){ // nếu chúng ta có một truy vấn đổi tên bảng
				if($ro=@db_query("select count(1) from `$oldTableName`")){ // nếu bảng cũ tồn tại, hãy đổi tên nó.
					$renameQuery=array_shift($arrAlter); // lấy và xóa truy vấn đổi tên
					echo "$renameQuery ... ";
					...
					if(is_array($arrAlter)) setupTable($tableName, $createSQL, false, $arrAlter); // thực hiện các truy vấn Alter trên bảng đã đổi tên ...
				}else{ // nếu tableName cũ không tồn tại (cũng không phải cái mới kể từ khi chúng ta ở đây), thì chỉ cần tạo bảng.
					setupTable($tableName, $createSQL, false); // không có truy vấn thay thế nào được thông qua ...
				}
			}else{ // tableName không tồn tại và không đổi tên, vì vậy chỉ cần tạo bảng
